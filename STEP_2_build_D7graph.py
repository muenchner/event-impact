...
def main():
    ...
    nx.write_gpickle(singleGraph,"testSimpleGraph.gpickle")
    print 'number of connected components: ',number_connected_components(DG.to_undirected())
    print DG.node[769701]['road']
    print DG.successors(769701)
    print DG.predecessors(769701)
    print len(DG.edges())
    print len(DG.nodes())
    print DG.edges()
    graphs=connected_component_subgraphs(singleGraph)
    counter=0
    for g in graphs:
        print g.nodes()
#        nx.draw(g )
#        title=str(counter)+'highwaymap.pdf'
#        plt.savefig(title)
#    nx.draw(DG)
    try:
        ...
        plt.xlim(-119.562,-117.508)
        plt.ylim(33.6075,34.795)
        plt.savefig("highwaymap2.pdf")
