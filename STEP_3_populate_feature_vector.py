...
def get_readings(chosen_sensor, route, time_of_day, chosen_date, cur):
    ...
    try:
        ...
        print chosen_date    
    try:
        cur.execute("select minutes, aveSpeed, aveFlow, length from sensors where sensorID=? and date=? and minutes>(?-1) and minutes<(?+16)", (sensor, chosen_date, time_of_day, time_of_day))
...
def get_init_readings(chosen_sensor, route, time_of_day, chosen_date, cur):
    """returns the flow-weighted average speed, occupancy, and flow across all lanes at one sensor as well as the occupancy at one sensor
    Args:
        chosen_sensor: id
    ...
    try:
        ...
        print chosen_date    
    try:
        cur.execute("select minutes, aveSpeed, aveOcc, aveFlow from sensors where sensorID=? and date=? and minutes>(?-1) and minutes<(?+16)", (sensor, chosen_date, time_of_day, time_of_day))
        ...
        if int(results[0]) == int(time_of_day):
            ...
            results.append(get_flow(chosen_sensor, time_of_day, chosen_date, cur))
            return results
    except TypeError:
        print chosen_sensor, chosen_date, time_of_day
...
def get_speed(chosen_sensor, time_of_day, chosen_date, cur):
    ...
    try:
        ...
        print time_of_day
        print chosen_date    
    cur.execute("select aveSpeed from sensors where sensorID=? and date=? and minutes=?", (sensor, chosen_date, time_of_day))
    try:
        v= cur.fetchone()[0]
...
def get_length(chosen_sensor, time_of_day, chosen_date, cur):
    ...
    if time_of_day%5==0:
        ...
        time_of_day=int(5.0*int(time_of_day/5)) #round down
    cur.execute("select length from sensors where sensorID=? and date=? and minutes=?", (int(chosen_sensor), chosen_date, time_of_day))
    try:
...
def get_flow(chosen_sensor, time_of_day, chosen_date, cur):
    ...
    if time_of_day%5==0:
        ...
        time_of_day=int(5.0*int(time_of_day/5)) #round down
    cur.execute("select aveFlow from sensors where sensorID=? and date=? and minutes=?", (int(chosen_sensor), chosen_date, time_of_day))
    try:
        return cur.fetchone()[0]
    except TypeError:
        print 'no flow'
def get_occ(chosen_sensor, time_of_day, chosen_date, cur):
    ...
    if time_of_day%5==0:
        ...
        time_of_day=int(5.0*int(time_of_day/5)) #round down
    cur.execute("select aveOcc from sensors where sensorID=? and date=? and minutes=?", (int(chosen_sensor), chosen_date, time_of_day))
    try:
        return cur.fetchone()[0]
    except TypeError:
        print 'no occ'
...
#        speedset.append([G.node[sensor]['mile'], v, vs, sensor, G.node[sensor]['road']])    
    ...
    for parent in parent_list:
        if len(newset)>0:
            ...
            try:
                if float(newset[-1])<100000: #in this case, delay was appended already to the end of newset
...
#    plt.show() #consider instead: http://networkx.lanl.gov/examples/graph/napoleon_russian_campaign.html
        ...
        pass
    pass
def main(G, rain_filename, db_file, tables, feature_vector_output_filename):
    #determine when it was raining and by how much
    rows=run_on_file(rain_filename)
    ...
    #find days with accidents
    dates=[]
    for row in cur.execute("select date from incidents where route==5 and direction =='S' and abs_postmile>0 and abs_postmile<130 and day>0 and day<6 and holiday_boolean==0 "): #check this postmile condition   
#    for row in cur.execute("select date from incidents where route==5 and direction =='S' and abs_postmile>127.30 and abs_postmile<127.32 and day>0 and day<6 and holiday_boolean==0 "): #check this postmile condition   
        dates.append(str(row[0]))
    num_incidents=len(dates)
    u_dates=f7(dates)
    u_dates=u_dates[-1:]
    print 'dates to analyze: ', u_dates
    print str(num_incidents)+' incidents here on '+str(len(set(dates)))+' days'
    
    #find when and where accidents start and then how they propagate
    for chosen_date in u_dates: #loop over days from d=1:k
        incident_list=[] #list of incidents on this day only
        start_tuples=[]
        for thing in cur.execute("select * from incidents where route==5 and direction=='S' and abs_postmile>0 and abs_postmile<130 and day>0 and day<6 and holiday_boolean==0 and date=?", (chosen_date,)): #check this postmile condition   
#        for thing in cur.execute("select * from incidents where route==5 and direction=='S' and abs_postmile>127.30 and abs_postmile<127.32 and day>0 and day<6 and holiday_boolean==0 and date=?", (chosen_date,)): #check this postmile condition   
...
#                            plot_v_series(speedset,t_min)
            ...
            if v_0 is None:
                ...
                if vs_0==50:
                    ...
                    v_dif=vs_0-v_0
            if (t_a_min+5)>1435: #hit midnight and need to rollover to new day!
                t5 = t_a_min+5-1440
                try:
                    ww=time.strptime(t_a_date, "%m/%d/%Y")
                    new=datetime(int(time.strftime("%Y", ww)),int(time.strftime("%m",ww)), int(time.strftime("%d", ww)))+relativedelta(days=+1)
                    date5=time.strftime("%m/%d/%Y", new.timetuple())                        
                    print 'Rolled over to new day ', date5
                    init_readings5=get_init_readings(u_a, accident_route, t5, date5, cur)
                except:
                    print 'failed to roll over ', relevant_date
                    init_readings5=(None, None, None)
            else:
                t5=t_a_min+5
                date5=t_a_date
                init_readings5=get_init_readings(u_a, accident_route, t5, date5, cur)
            ...
            q_05=init_readings5[2]
            vs_05=get_vS(u_a, t5, cur)
            if rho_05 is None:
                rho_0='NA'
            if q_05 is None:
                q_05='NA'
            if v_05 is None:
                ...
                if vs_05==50:
                    ...
                    v_dif5=vs_05-v_05
            if (t_a_min+10)>1435: #hit midnight and need to rollover to new day!
                try:
                    t10=t_a_min+10-1440
                    ww=time.strptime(t_a_date, "%m/%d/%Y")
                    new=datetime(int(time.strftime("%Y", ww)),int(time.strftime("%m",ww)), int(time.strftime("%d", ww)))+relativedelta(days=+1)
                    date10=time.strftime("%m/%d/%Y", new.timetuple())                        
                    print 'Rolled over to new day ', date10
                    init_readings10=get_init_readings(u_a, accident_route, t10, date10, cur)
                except:
                    print 'failed to roll over ', relevant_date
                    init_readings10=(None, None, None)
            else:
                t10=t_a_min+10
                date10=t_a_date
                init_readings10=get_init_readings(u_a, accident_route, t10, date10, cur)
            ...
            q_010=init_readings10[2]
            vs_010=get_vS(u_a, t10, cur)
            if rho_010 is None:
                rho_010='NA'
            if q_010 is None:
                q_010='NA'
            ...
            if v_0u is None:
                ...
                if vs_0u==50:
                    ...
                    v_difu=vs_0u-v_0u          
            init_readings5u=get_init_readings(u_a_up, accident_route, t5, date5, cur)
            rho_05u=init_readings5u[1]
            v_05u=init_readings5u[0]
            q_05u=init_readings5u[2]
            vs_05u=get_vS(u_a_up, t5, cur)
            if rho_05u is None:
                rho_0u='NA'
            if q_05u is None:
                q_0u='NA'
            ...
            init_readings10u=get_init_readings(u_a_up, accident_route, t10, date10, cur)
            rho_010u=init_readings10u[1]
            v_010u=init_readings10u[0]
            q_010u=init_readings10u[0]
            vs_010u=get_vS(u_a_up, t10, cur)
            if rho_010u is None:
                rho_010u='NA'
            if q_010u is None:
                q_010u='NA'
            ...
            u_a_down=get_downstream_sensors(u_a, G)[0][0]
            print 'down',  u_a_down
            init_readingsd=get_init_readings(u_a_down, accident_route, t_a_min, t_a_date, cur)
            rho_0d=init_readingsu[1]
            v_0d=init_readingsu[0]
            q_0d=init_readingsu[2]
            vs_0d=get_vS(u_a_down, t_a_min, cur)
            if rho_0d is None:
                rho_0d='NA'
                rho_dif='NA'
            if v_0d is None:
            ...
            #bring together information calculated in main feature vector with the extra post-processed steps
            try:    
                if t_min-t_a_min<0:
                    total_duration = 1440 + t_min-t_a_min
                else:
                    total_duration = t_min-t_a_min
                feature_vector_line=str(minutes)+'\t'+str(accident_pm)+'\t'+str(day)+'\t'+str(row[13])+'\t'+str(rain)+'\t'+str(rainy_day)+'\t'+str(lanes)+'\t'+str(rho_dif)+'\t'+str(rho_0)+'\t'+str(v_0)+'\t'+str(q_0)+'\t'+str(v_dif)+'\t'+str(rho_05)+'\t'+str(v_05)+'\t'+str(q_05)+'\t'+str(v_dif5)+'\t'+str(rho_010)+'\t'+str(v_010)+'\t'+str(q_010)+'\t'+str(v_dif10)+'\t'+str(rho_0u)+'\t'+str(v_0u)+'\t'+str(q_0u)+'\t'+str(v_difu)+'\t'+str(rho_05u)+'\t'+str(v_05u)+'\t'+str(q_05u)+'\t'+str(v_dif5u)+'\t'+str(rho_010u)+'\t'+str(v_010u)+'\t'+str(q_010u)+'\t'+str(v_dif10u)+'\t'+str(sum(inc_delay))+'\t'+str(sum(tot_delay))+'\t'+str(row[4])+'\t'+str(total_duration)+'\t'+str(adjacent)+'\n' #TODO
            except:
                try:
                    feature_vector_line=str(minutes)+'\t'+str(accident_pm)+'\t'+str(day)+'\t'+str(row[13])+'\t'+str(rain)+'\t'+str(rainy_day)+'\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\t'+str(sum(inc_delay))+'\t'+str(sum(tot_delay))+'\t'+str(row[4])+'\t'+str(t_min-t_a_min)+'\t'+str(adjacent)+'\n'
                except:
                    feature_vector_line='NA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\n'    
            print feature_vector_line
            
            #write this finalized feature vector line to the output file
            try:
                output_file=open(feature_vector_output_filename, "a")
...
if __name__ == '__main__':
    G=nx.read_gpickle("testSimpleGraphd7v2.gpickle")
    rain_filename = 'wwp_accumulated_rain.txt'
    db_file = 'd7Huge.db'
    tables=['sensors','vStarValsMedianWeek', 'incidents', 'devices']
    feature_vector_output_filename = '20120205_featureVector_d7vendend.txt'
    main(G, rain_filename, db_file, tables, feature_vector_output_filename)
